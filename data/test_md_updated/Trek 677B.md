...
|Drift with Temp|Less than 350 ppm/°C|
...
- High Voltage On/Off
- Current Limit
- Current Limit Set Accuracy
- Supply Mode Voltage Control

An open collector, TTL compatible input to turn on and off the high voltage when the High Voltage switch is in the Remote position.

A three-position rocker switch to select ON, OFF, or REMOTE.

Adjustable from 0 to ±5 mA. A multiturn control is used to set the current limit as indicated by the digital display.

An amber LED will illuminate when the instrument is in a current limit condition.

Better than 1% of setting.

# Voltage Selection

A multiturn control to set the desired output voltage as indicated by the digital display.
